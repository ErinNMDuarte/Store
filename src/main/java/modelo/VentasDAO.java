...
public class VentasDAO {
	...
	public boolean insertarventa(VentasDTO ven) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "se inserto venta");
	...
	public VentasDTO consultarcodventa() {
			try {
				...
				if(rs.next()) {
					...
					vendto=new VentasDTO(rs.getInt(1), rs.getLong(2), rs.getLong(3), rs.getDouble(4), rs.getDouble(5), rs.getDouble(6));
